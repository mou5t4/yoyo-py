#!/usr/bin/env python3
"""
Demo application for Playlist browsing with YoyoPod.

This demo connects to a Mopidy server and allows browsing and
loading Spotify playlists through the PlaylistScreen interface.
...
from yoyopy.ui.screens import NowPlayingScreen, MenuScreen, PlaylistScreen
...
def main():
    ...
    # Create menu screen with playlist browser option
    menu_items = ["Browse Playlists", "Now Playing", "Back"]
    menu_screen = MenuScreen(display, context, items=menu_items)

    # Create playlist browser screen with Mopidy client
    ...
    screen_manager.register_screen("playlists", playlist_screen)
    ...
    logger.info("")
    logger.info("  Playlist Browser:")
    logger.info("    A - Load selected playlist")
    logger.info("    B - Back to menu")
    logger.info("    X/Y - Navigate playlists")
    logger.info("")
